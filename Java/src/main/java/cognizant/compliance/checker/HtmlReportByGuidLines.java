...
public class HtmlReportByGuidLines {
	...
	public String getHtmlString() throws IOException {
		...
		html.append(
				"<h1 class=\"mt-4 mb-3\"><center>Accessibility Test Run Report</center></h1><div class=\"container\">"
						+ "<div class=\"alert alert alert-info\">Test Run: " + startDateTime + " - " + endDateTime
						+ "</div>" + getTotalIssuesHtml(statistics, axeViolations) + "</div>");
		...
		for (Map.Entry<String, WcagStatistics> entry : statistics.entrySet()) {
			...
			html.append(groupJsonByGuidLine(pageKey, report, pageIndex));
	...
	public String groupJsonByGuidLine(String pageTitle, WcagViolations violations, Integer pageIndex) {
		...
		for (int i = 0; i < violations.size(); i++) {
			...
			Map<String, String> guideLines = getGuideLine(violation);

			for (Map.Entry<String, String> guideLine : guideLines.entrySet()) {
				resultMap.putIfAbsent(guideLine.getValue(), new StringBuilder());
				resultMap.get(guideLine.getValue()).append("<li>" + violation.ElementXPath + "</li>");
			}
		}
		...
		for (Map.Entry<String, StringBuilder> entry : resultMap.entrySet()) {
			...
			tempHtml.append(appendErrorsForGuidLines(violations, xpathList, key, pageIndex));
		}

		return tempHtml.toString();
	}

	public String appendErrorsForGuidLines(WcagViolations violations, String xpathList, String filterValue,
			Integer pageIndex) {
		StringBuilder tempHtml = new StringBuilder();
		...
		for (int i = 0; i < violations.size(); i++) {
			...
			String guideLineCheckList = "";

			Map<String, String> guideLines = getGuideLine(violation);

			if (guideLines.containsValue(filterValue)) {
				...
				for (Map.Entry<String, String> guideLine : guideLines.entrySet()) {
					if (!filterValue.equals("None") && guideLine.getValue().equals(filterValue)) {
						guideLineCheckList = guideLineCheckList + "<a href=\"" + guideLine.getKey() + "\">"
								+ guideLine.getValue() + "</a><br>";
					}
				}

				if (guideLineCheckList == "") {
					guideLineCheckList = "N/A";
				}

				String navigation = filterValue.replace("(", "").replace(")", "").replace(".", "").replace("-", "")
						.replace(" ", "");

				String htmlString = htmlAccordion.replace("${index}", navigation + categoryCode + "_" + pageIndex)
						.replace("${errorBgColor}", errorBgColor).replace("${title}", filterValue)
						.replace("${errorType}", title).replace("${summary}", summary).replace("${purpose}", purpose)
						.replace("${actions}", actions).replace("${guideLineCheckList}", guideLineCheckList)
						.replace("${xpathlist}", xpathList).replace("${highImpactErrorCountMsg}", impactCategoryMsg);
	...
	public Map<String, String> getGuideLine(WcagViolation violation) {
		...
		if (guideLines.size() >= 0) {
			for (int i = 0; i < guideLines.size(); i++) {
				...
				guidLineList.put(guideLine.link, guideLine.name);
			}
		} else {
