...
namespace Reports.Html.Reporter
{
    ...
    public class HtmlReportByGuideline
    {
        ...
        private static string htmlAccordion = "<div class=\"accordion\" id=\"accordionPanelsStayOpen${index}\">"
                ...
                + "						   <div class=\"col-2\">${highImpactErrorCountMsg}</div>"
                + "                           </div></div></div>"
        ...
        private static string htmlAccordionForNoneFirstItem = "<div class=\"accordion\" id=\"accordionPanelsStayOpen${index}\">"
                + "					<div class=\"accordion-item\">"
                + "					<div class=\"accordion-header\" id=\"heading${index}\">"
                + "					<div class=\"accordion-button text-white text-opacity-75 ${errorBgColor}\" type=\"button\" data-bs-toggle=\"collapse\" data-bs-target=\"#panelsStayOpen-collapse${index}\" aria-expanded=\"true\" aria-controls=\"panelsStayOpen-collapse${index}\">"
                + "						<div class=\"row col-12 g-2\">"
        ...
        private static string htmlAccordionForNoneItems = "<div class=\"fw-bold mt-3 text-secondary\"><h3>${errorType}</h3></div><br>"
               + "							<div class=\"fw-bold mt-3 text-secondary\">What It Means:</div>"
               + "							<div class=\"card-text mt-3\">${summary}</div>"
               + "							<div class=\"fw-bold mt-3 text-secondary\">Why It Matters:</div>"
               + "							<div class=\"card-text mt-3\">${purpose}</div>"
               ...
               + "							<div class=\"card-text mt-3\">${actions}</div>"
               + "							<div class=\"fw-bold mt-3 text-secondary\">Item's XPath:</div>"
               + "							<ul class=\"mt-3\"><small>${xpath}</small></ul>";

        private static string htmlAccordionForNoneLastLine = "</div></div></div></div>";
        ...
        public static string GetHtmlstring()
        {
            ...
            foreach (var item in statsData)
            {
                ...
                if (errorCount > 0)
                {
                    ...
                    html.Append(GroupJsonByGuideLine(report, "error", pageIndex));
                }

                if (contrastCount > 0)
                {
                    html.Append(GroupJsonByGuideLine(report, "contrast", pageIndex));
                }

                if (alertsCount > 0)
                {
                    html.Append(GroupJsonByGuideLine(report, "alert", pageIndex));
                }

                if (seriousCount > 0)
                {
                    html.Append(GroupJsonByGuideLine(report, "serious", pageIndex));
                }

                if (criticalCount > 0)
                {
                    html.Append(GroupJsonByGuideLine(report, "critical", pageIndex));
                }

                if (moderateCount > 0)
                {
                    html.Append(GroupJsonByGuideLine(report, "moderate", pageIndex));
                }

                //if (ariaCount > 0)
                //{
                //    html.Append(GroupJsonByGuideLine(report, "aria", pageIndex));
                //}

                //if (featureCount > 0)
                //{
                //    html.Append(GroupJsonByGuideLine(report, "feature", pageIndex));
                //}

                //if (structureCount > 0)
                //{
                //    html.Append(GroupJsonByGuideLine(report, "structure", pageIndex));
                //}
        ...
        private static string GroupJsonByGuideLine(List<AccessibilityResult> data, string category, int pageIndex)
        {
            ...
            foreach (var report in reportByCategory)
            {
                ...
                if (report.GuideLines == null || report.GuideLines.Count == 0)
                {
                    if (!resultMap.ContainsKey("None"))
                    {
                        resultMap.Add("None", new StringBuilder());
                    }
                    resultMap["None"].Append("<li>" + report.ElementXPath + "</li>");
                    key = "None";
                }
                else
                {
                    foreach (var guideLine in report.GuideLines)
                    {
                        key = guideLine.Name + " - " + guideLine.GuidelineLevel;
                        if (!string.IsNullOrEmpty(key))
                        {
                            if (!resultMap.ContainsKey(key))
                            {
                                resultMap.Add(key, new StringBuilder());
                            }

                            resultMap[key].Append("<li>" + report.ElementXPath + "</li>");
                        }
                    }
                }
            }
            ...
            {
                var reports = new List<AccessibilityResult>();

                if (key.Equals("None"))
                {
                    reports = data.FindAll(c => c.Type == category && c.GuideLines == null || c.GuideLines.Count() == 0);
                    tempHtml.Append(AppendErrorsForNoneTypes(reports, resultMap[key].ToString(), pageIndex));
                }
                else
                {
                    reports = data.FindAll(c => c.Type == category && c.GuideLines.Any(x => (x.Name + " - " + x.GuidelineLevel).Equals(key)));
                    tempHtml.Append(AppendErrorsForGuidelines(reports, resultMap[key].ToString(), key, pageIndex));
                }
            }
        ...
        private static string AppendErrorsForNoneTypes(List<AccessibilityResult> items, string xpathList, int pageIndex)
        {
            var tempHtml = new StringBuilder();
            var count = xpathList.Split("<li>").Length - 1;
            var impactCategoryMsg = string.Empty;

            var errorBgColor = "bg-warning";
            impactCategoryMsg = count + " medium impact";
            totalMediumIssuesCount = totalMediumIssuesCount +count;

            string guideLine = "None";
            var htmlString = htmlAccordionForNoneFirstItem.Replace("${index}", $"{guideLine}_{pageIndex}")
                .Replace("${errorBgColor}", errorBgColor)
                .Replace("${title}", guideLine)
                .Replace("${highImpactErrorCountMsg}", impactCategoryMsg);
                
            tempHtml.Append(htmlString);

            var errorTypes = items.GroupBy(x => x.Title)
                .Select(grp=> grp.ToList())
                .ToList();

            foreach(var error in errorTypes)
            {
                var xpaths = string.Join(string.Empty, error.Select(x => "<li>"+x.ElementXPath+ "</li>"));
           
                htmlString = htmlAccordionForNoneItems
                    .Replace("${errorType}", error[0].Title)
                    .Replace("${summary}", error[0].Summary)
                    .Replace("${purpose}", error[0].Purpose)
                    .Replace("${actions}", error[0].Actions)
                    .Replace("${xpath}", xpaths);

                tempHtml.Append(htmlString);
            }

            tempHtml.Append(htmlAccordionForNoneLastLine);

            return tempHtml.ToString();
        }

        private static string AppendErrorsForGuidelines(List<AccessibilityResult> items, string xpathList, string guideLine, int pageIndex)
        {
            var tempHtml = new StringBuilder();
            var count = xpathList.Split("<li>").Length - 1;
            var impactCategoryMsg = string.Empty;
            ...
            foreach (var item in items)
            {
                ...
                if (!guideLine.Equals("None"))
                {
                    foreach (var guidLine in item.GuideLines)
                    {
                        guideLinestring = guideLinestring + "<a href=\"" + guidLine.GuidelineLink + "\">"
                           + guidLine.Name + " - " + guidLine.GuidelineLevel + "</a><br>";
                    }
                }

                var navigation = guideLine.Replace("(", "").Replace(")", "").Replace(".", "").Replace("-", "").Replace(",", string.Empty).Replace(" ", "");
                var title = item.Title.Replace("(", "").Replace(")", "").Replace(",", string.Empty).Replace(" ", "").Replace(".", "").Replace("-", "");
